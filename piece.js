...
let noteStartY = 0;
let noteStartTranslate = 0;
let noteTranslateY = 0;

const fixedContent = document.querySelector(".fixed-note-content");
...
document.addEventListener("mousemove", e => {
  ...
  if (noteDragging && fixedContent) {
    console.log("dragging");
    noteTranslateY = 
    noteStartTranslate + (e.clientY - noteStartY);

    fixedContent.style.transform =
      `translateY(${noteTranslateY}px)`;
  }
});
...
function moveResetButton() {
  const buttonSize = resetButton.offsetWidth;

  buttonX += buttonVX;
  buttonY += buttonVY;

  if (buttonX <= 0 || buttonX >= window.innerWidth - buttonSize) {
    buttonVX *= -1;
  }

  if (buttonY <= 0 || buttonY >= window.innerHeight - buttonSize) {
    buttonVY *= -1;
  }
  ...
  requestAnimationFrame(moveResetButton);
}
